...
def draw_figure_1_combined(
    ...
    ax.set_title(f"2010–2024年影响台风路径 (总数: {total_nc}个)", fontsize=16)
...
def draw_figure_2_highlight_only(
    ...
    ax.set_title(f"2010–2024年台风大风影响时段路径 (共 {total_excel} 个)", fontsize=16)
...
def generate_statistics_output(analysis_df: pd.DataFrame, output_dir: str):
    ...
    ax.set_title(f"2010-2024年浙江省大风影响台风个数年分布 (总个数: {total_events})") 
    ax.set_xlabel("年份")
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    month_counts.reindex([f"{i}月" for i in range(1, 13)]).plot(kind='bar', ax=ax, title="2010-2024年浙江省大风影响台风月分布", rot=0, color=COLOR_MONTH)
    ax.set_title(f"2010-2024年浙江省大风影响台风月分布 (总个数: {total_events})") 
    ax.set_xlabel("月份")
    ax.set_ylabel("台风个数")
    ...
    ax.set_title(f"2010-2024年浙江省大风影响台风强度 (总个数: {total_events})") 
    ...
    ax.set_title(f"2010-2024年浙江省大风影响台风\n不同强度台风的平均大风影响时长 (总个数: {total_events})")
    ax.set_xlabel("影响时段内的最强强度")
    ...
    try:
        ...
        ax.set_title(f"2010-2024年浙江省大风影响台风的 月份-强度 分布 (总个数: {total_events})")
        ax.set_xlabel("影响时段内的最强强度")
        ax.set_ylabel("开始月份")
    ...
    try:
        ...
        ax.set_title(f"2010-2024年浙江省大风影响台风的 强度-时长 分布 (总个数: {total_events})")
        ax.set_xlabel("影响时段内的最强强度")
