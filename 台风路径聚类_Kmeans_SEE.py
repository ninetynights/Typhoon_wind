...
- 聚类：弧长重采样→相邻增量向量化→KMeans，K 由轮廓系数在候选里自动选择；
- 输出：
...
import os, re, math, json, importlib.util, types, sys, hashlib
...
OUT_DIR       = os.path.join(os.path.dirname(EXCEL_PATH), "输出_影响段聚类")
...
# 浙江及周边的地图范围
EXTENT = [110, 135, 15, 40]  # [lon_min, lon_max, lat_min, lat_max]
...
def load_vis_module(path: str) -> types.ModuleType:
    """按文件路径加载可视化脚本为一个 *已注册* 的模块，避免 dataclass 在导入期
    访问 sys.modules 失败（你遇到的 AttributeError: 'NoneType'... 就是这个）。"""
    # 为该路径生成一个稳定且不冲突的模块名
    ...
    # 关键：在 exec_module 前写入 sys.modules，供 dataclasses/typing 在导入期查找
...
def vectorize_segments(segments: List[ImpactSeg], P=PARAMS):
    ...
    if not X:
        return np.zeros((0, 2*(P["resample_N"]-1))), [], []
...
def choose_k_and_cluster(X: np.ndarray, k_candidates: List[int]):
    ...
    for k in k_candidates:
        ...
        try: s = silhouette_score(Xs, labels)
        except Exception: s = -1.0
        if s > best[0]: best = (s, k, km, labels)
    if best[1] is None:
        k = max(2, min(4, len(X)-1))
        km = KMeans(n_clusters=k, n_init=50, random_state=42)
        labels = km.fit_predict(Xs)
        return labels, k, float('nan'), km
    return best[3], best[1], best[0], best[2]

def plot_clusters(segments: List[ImpactSeg], labels_map: Dict[int,int], out_png: str):
    if HAS_CARTOPY:
    ...
    for i, seg in enumerate(segments):
        ...
        xs = [p.lon for p in seg.pts]
        ys = [p.lat for p in seg.pts]
        if HAS_CARTOPY:
            ax.plot(xs, ys, '-', lw=2.2 if cid>=0 else 1.2, color=color, alpha=0.95 if cid>=0 else 0.6, transform=ccrs.PlateCarree())
        else:
            ax.plot(xs, ys, '-', lw=2.2 if cid>=0 else 1.2, color=color, alpha=0.95 if cid>=0 else 0.6)
        counts[cid] = counts.get(cid, 0) + 1

    plt.title('大风影响段路径聚类（仅 Excel 编号）')
    plt.tight_layout()
    os.makedirs(OUT_DIR, exist_ok=True)
    plt.savefig(out_png, dpi=180)
    plt.close()

# 评估 K 的工具：SSE(肘部) + Silhouette
import matplotlib.pyplot as plt

def evaluate_k_grid(X: np.ndarray, k_list, random_state=42):
    scaler = StandardScaler(); Xs = scaler.fit_transform(X)
    rows = []
    for k in k_list:
        if k >= len(Xs):
        ...
        try:
            ...
            sil = float('nan')
        rows.append({"K": int(k), "SSE": sse, "Silhouette": sil})
        print(f"[K={k}] SSE={sse:.2f}, Silhouette={sil:.3f}")
    return pd.DataFrame(rows).sort_values("K").reset_index(drop=True)

def pick_k_by_elbow(df: pd.DataFrame):
    K = df["K"].to_numpy(); SSE = df["SSE"].to_numpy()
    if len(K) < 3:
        return (np.nan, np.nan)
    rel_drop = np.r_[np.nan, (SSE[:-1]-SSE[1:]) / np.maximum(SSE[:-1], 1e-9)]
    elbow1 = np.nan
    for i in range(2, len(K)):
        if rel_drop[i-1] >= 0.15 and rel_drop[i] < 0.10:
            elbow1 = int(K[i]); break
    def angle(p0, p1, p2):
        v1 = p0 - p1; v2 = p2 - p1
        a = np.arctan2(v1[1], v1[0]); b = np.arctan2(v2[1], v2[0])
        ang = abs(a-b);  ang = 2*np.pi - ang if ang > np.pi else ang
        return ang
    elbows = []
    for i in range(1, len(K)-1):
        p0 = np.array([K[i-1], SSE[i-1]]); p1 = np.array([K[i], SSE[i]]); p2 = np.array([K[i+1], SSE[i+1]])
        elbows.append(angle(p0,p1,p2))
    elbow2 = int(K[1+int(np.argmin(elbows))]) if elbows else np.nan
    return (elbow1, elbow2)

def plot_k_curves(df: pd.DataFrame, out_png: str):
    fig, ax1 = plt.subplots(figsize=(9,5)); ax2 = ax1.twinx()
    ax1.plot(df["K"], df["SSE"], marker='o', lw=1.8, label='SSE')
    ax1.set_xlabel('K'); ax1.set_ylabel('SSE')
    ax2.plot(df["K"], df["Silhouette"], marker='s', ls='--', lw=1.8, label='Silhouette')
    ax2.set_ylabel('Silhouette')
    lines = ax1.get_lines() + ax2.get_lines(); labels = [l.get_label() for l in lines]
    ax1.legend(lines, labels, loc='best')
    plt.title('K 评估曲线：SSE（肘部）与 Silhouette')
    plt.tight_layout(); os.makedirs(os.path.dirname(out_png), exist_ok=True); plt.savefig(out_png, dpi=180); plt.close()
...
def main():
    ...
    target_tids = set(dfx[ID_COL].astype(str).str.zfill(4))

    # 只提取这些编号的轨迹，逐小时插值（复用可视化脚本）
    ...
    for idx, row in dfx.iterrows():
        ...
        st  = pd.to_datetime(row[ST_COL])
        en  = pd.to_datetime(row[EN_COL])
        ...
        arc = 0.0
        for i in range(1, len(coords)):
            arc += haversine_km(coords[i-1][0], coords[i-1][1], coords[i][0], coords[i][1])

        participate = (len(coords) >= PARAMS["min_points_for_cluster"]) and (arc >= PARAMS["min_arclength_km"])
        segments.append(ImpactSeg(idx=idx, tid=tid, start=st, end=en, pts=pts_win))
        rows.append({
            "idx": idx, "tid": tid, "start": st, "end": en,
            "found_in_lib": bool(pts_all),
            "win_points": len(coords),
    ...
    # —— K 评估：SSE + Silhouette ——（评估、曲线、自动建议 K）
    k_eval = list(range(3, 16))
    k_eval = [k for k in k_eval if k < X.shape[0]]  # 不能 >= 样本数
    df_k = evaluate_k_grid(X, k_eval)

    eval_csv = os.path.join(OUT_DIR, "影响段_K评估_SSE_Silhouette.csv")
    df_k.to_csv(eval_csv, index=False, encoding="utf-8-sig")
    print("→ 已输出 K 评估表：", eval_csv)

    eval_png = os.path.join(OUT_DIR, "影响段_K评估曲线.png")
    plot_k_curves(df_k, eval_png)
    print("→ 已输出 K 评估曲线：", eval_png)

    k_elbow1, k_elbow2 = pick_k_by_elbow(df_k)
    cands = []
    for k in (k_elbow1, k_elbow2):
        if pd.notna(k):
            cands.extend([int(k)-1, int(k), int(k)+1])
    cands = sorted(set([k for k in cands if k in df_k["K"].tolist()]))

    if not cands:
        k_best = int(df_k.loc[df_k["Silhouette"].idxmax(), "K"])
        print(f"未检测到明显肘部，采用 Silhouette 最大的 K={k_best}")
    else:
        sub = df_k[df_k["K"].isin(cands)]
        k_best = int(sub.loc[sub["Silhouette"].idxmax(), "K"])
        best_sil = float(sub.loc[sub["K"]==k_best, "Silhouette"])
        print(f"建议 K（肘部±1 且兼顾 Silhouette）：K={k_best}；候选={cands}，Silhouette@K={k_best}={best_sil:.3f}")
    ...
    labels = km.fit_predict(Xs)
    final_sil = silhouette_score(Xs, labels)
    print(f"最终采用 K={k_best}，Silhouette={final_sil:.3f}")
    ...
    plot_clusters(segments, idx2cid, png_path)
    print("→ 聚类成图：", png_path)
