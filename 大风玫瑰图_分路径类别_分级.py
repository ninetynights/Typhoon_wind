...
- [新] 聚合分析：为每个路径类别，额外生成一个“总体风向玫瑰图” (聚合所有站点) 及其CSV。
...
# 2. 基础输出目录 (不含风力等级)
BASE_OUTPUT_DIR = Path("/Users/momo/Desktop/业务相关/2025 影响台风大风/测试")
...
# --- [NEW] 新增：用于绘制聚合风玫瑰图的函数 ---
def plot_aggregated_rose(counts, edges_deg, norm, cmap, title, save_path, global_vmax):
    """
    [NEW] 绘制一个单独的、聚合的风玫瑰图 (标准极坐标图)
    
    Args:
        counts (np.array): 1D 数组 (size N_BINS)，包含每个扇区的总小时数
        edges_deg (np.array): 角度分箱的边界 (size N_BINS + 1)
        norm (mcolors.Norm): 用于映射颜色的标准化器
        cmap (matplotlib.Colormap): 颜色图
        title (str): 图表标题
        save_path (Path): 图像保存路径
        global_vmax (float): 用于色标的最大值
    """
    N_BINS = len(counts)
    if N_BINS == 0 or np.sum(counts) == 0:
        print(f"聚合数据为空，跳过绘图: {save_path.name}")
        return
        
    edges_rad = np.deg2rad(edges_deg)
    angles_rad = edges_rad[:-1] # 16个起始角度
    widths_rad = np.diff(edges_rad) # 16个宽度
    
    fig, ax = plt.subplots(figsize=(10, 8), subplot_kw={'projection': 'polar'})
    
    # 使用传入的 norm 和 cmap 来上色
    colors = cmap(norm(counts))
    
    # 绘制柱状图
    # x 是每个扇区的起始角度
    ax.bar(angles_rad, counts, width=widths_rad, 
           color=colors, 
           edgecolor='black', 
           linewidth=0.5, 
           zorder=10,
           align='edge') # 'edge' 表示柱子从 angles_rad 开始绘制
    
    # 格式化
    ax.set_theta_zero_location('N') # 0度(N)在顶部
    ax.set_theta_direction(-1) # 顺时针
    ax.set_rlabel_position(90) # 半径标签放在90度位置
    
    # 设置角度刻度标签
    ticks = np.linspace(0, 360, 8, endpoint=False)
...
def analyze_and_plot_rose_by_cluster(min_threshold, max_threshold, level_name):
    ...
    # 1. [MODIFIED] 根据风力等级动态创建输出目录 (增加聚合目录)
    ...
    # 站点详细输出
    csv_out_dir = current_output_dir / "csv_station_detail"
    fig_out_dir = current_output_dir / "figs_station_maps"
    
    # --- [NEW] 聚合输出 ---
    csv_agg_out_dir = current_output_dir / "csv_cluster_aggregated"
    fig_agg_out_dir = current_output_dir / "figs_cluster_aggregated"
    
    ensure_dir(csv_out_dir)
    ensure_dir(fig_out_dir)
    ensure_dir(csv_agg_out_dir) # [NEW]
    ensure_dir(fig_agg_out_dir) # [NEW]
    ...
    for cid in cluster_ids:
        ...
        if rose_counts_list:
            ...
            # [MODIFIED] GLOBAL_VMAX 应该基于 *单个扇区* 的最大值, 保持不变
    ...
    for cid in cluster_ids:
        ...
        rose_counts_matrix = data["matrix"] # (N_stations, N_BINS)
        ...
        csv_path = csv_out_dir / f"Cluster_{cid}_StationDetail_Rose.csv"
        df_csv.to_csv(csv_path, index=False, encoding='utf-8-sig')
        print(f"已保存 [站点详细] 风玫瑰数据到: {csv_path}")

        # --- 6b. [NEW] 计算、保存和绘制聚合风玫瑰 ---
        print(f"--- 正在为类别 {cid} (风力 {level_name}) 生成 [聚合] 数据 ---")
        
        # 1. [NEW] 计算聚合数据 (按列求和)
        # rose_counts_matrix 是 (N_stations, 16_bins)
        # aggregated_counts 是 (16_bins,)
        aggregated_counts = np.sum(rose_counts_matrix, axis=0)
        
        # 2. [NEW] 保存聚合CSV
        df_agg = pd.DataFrame([aggregated_counts], columns=bin_headers)
        df_agg.insert(0, 'cluster_id', cid)
        df_agg['total_hours'] = np.sum(aggregated_counts)
        
        csv_agg_path = csv_agg_out_dir / f"Cluster_{cid}_Aggregated_Rose.csv"
        df_agg.to_csv(csv_agg_path, index=False, encoding='utf-8-sig')
        print(f"已保存 [聚合] CSV到: {csv_agg_path}")

        # 3. [NEW] 绘制聚合风玫瑰图
        agg_title = f"路径类别 {cid} 总体风向玫瑰图 ({level_name})\n(所有站点总计)"
        agg_fig_path = fig_agg_out_dir / f"Cluster_{cid}_Aggregated_Rose_Plot.png"
        
        plot_aggregated_rose(
            counts=aggregated_counts,
            edges_deg=EDGES_DEG,
            norm=norm, # 使用与地图相同的 norm 和 cmap
            cmap=cmap,
            title=agg_title,
            save_path=agg_fig_path,
            global_vmax=GLOBAL_VMAX # 传入最大值用于色标标签
        )
        
        # --- 6c. [原功能] 绘制站点分布地图 ---
        print(f"--- 正在为类别 {cid} (风力 {level_name}) 生成 [站点地图] ---")
        ...
        ax.set_title(f"路径类别 {cid} 的{level_name}大风风玫瑰图 (站点分布)", fontsize=16, pad=20)

        fig_path = fig_out_dir / f"Cluster_{cid}_StationDetail_Map.png"
        fig.savefig(fig_path, dpi=200, bbox_inches='tight')
        print(f"已保存 [站点地图] 到: {fig_path}")
        plt.close(fig)
